...
const Testimonials = () => {
    ...
    const [monitoredCounter, setMonitoredCounter] = useState(0);
    const [trigger, setTrigger] = useState(false)


    useEffect(() => {
        setSubscriberCount(0);
        setMonitoredCounter(0)
    }, [trigger])

    useEffect(() => {
        const timer = setTimeout(() => {
            if (subscriberCount < 12) {
                setSubscriberCount(subscriberCount + 0.8);
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (monitoredCounter <= 4.2) {

                setMonitoredCounter(monitoredCounter + 0.2)
            }
        }, 100)
    ...
    window.addEventListener('scroll', () => {
        document.getElementById("facts").getBoundingClientRect().top > document.documentElement.clientHeight ? setTrigger(true) : setTrigger(false)
    })
    ...
    const reviews = [
        ...
        },
        {
            ...
            image: imad,
            location: "Gujrat",
            description: "Growcial bull academy was an excellent 👍 decision for me It proved to be life changing for me,<br /> I am very happy that I am also a member of Growcial Bull. ",
            rating: fiveStar
        },
        {
            id: 17,
    ...
    return (
        <div className='bg-transparent bg-[length:cover] lg:bg-[length:46%]' id={styles.container}>
            <div className='pb-[70px] px-[29px] lg:py-[63px] lg:px-[140px]' id='facts'>
                <div className="hidden lg:flex flex-col-reverse lg:flex-row lg:gap-[120px] items-center justify-start lg:justify-between">
                    {facts.map(({ heading, description }, id) => <Fact
                        key={id}
                        id={id}
                        heading={heading}
                ...
                <div className='lg:hidden'>
                    <h1 className='text-[30px] lg:text-[80px] font-bold text-[#8383FF] drop-shadow-[0_0_6px]'>Our Fact</h1>
                    <p className='text-[18px] lg:text-[20px] font-medium'>We are already having many Traders to make profits in Market on Daily basis, And Banknifty tips is simply the best choice for all traders.The best Banknifty tips provider in the Market.</p>

                    <div className="flex justify-around">
                        <div className='text-center'>
                            <h1 className='text-[48px] lg:text-[80px] font-bold text-[#8383FF] drop-shadow-[0_0_6px]'>{subscriberCount.toFixed(1)}K</h1>
                            <p className='text-[12px] lg:text-[20px] font-medium'>Our Subscribers</p>

                        </div>
                        <div className='text-center'>
                            <h1 className='text-[48px] lg:text-[80px] font-bold text-[#8383FF] drop-shadow-[0_0_6px]'>{monitoredCounter.toFixed(1)}K</h1>
...
{/* m-auto */}
            <div className='relative' id="testimonials">
                <img src={comma} className='absolute w-63px top-[76%] bottom-0  h-63px left-[23%]  rotate-180' alt="" />
                <div id={`${styles.testimonials}`} className='pt-[68px] testimonial lg:px-[260px] bg-[length:200%] bg-center lg:bg-[length:32%]'>
                    <h1 className='text-[22px] lg:text-[40px] font-bold text-center text-[#8383FF] drop-shadow-[0_0_6px] uppercase'>Testimonials</h1>


                    <OwlCarousel className='owl-theme z-0' dots={false} nav loop items={1} animateIn="animate__animated animate__bounceInRight" animateOut="animate__animated animate__slideOutLeft">
                        {reviews.map(item => <Testimonial item={item} />)}
                    </OwlCarousel>
                </div>
                <img src={comma} className='absolute w-63px top-[68%] bottom-[0%]  h-63px right-[23%]   rotate-360' alt="" />
            </div>
        </div >
    );
};
